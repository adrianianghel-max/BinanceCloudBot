...
from state_manager import get_alert_state, should_send_only_new, update_alert_state
...
def with_retries(func, *args, **kwargs):
    ...
    for attempt in range(1, config.MAX_RETRIES + 1):
        ...
        except (ccxt.RateLimitExceeded, ccxt.NetworkError, ccxt.ExchangeNotAvailable, ccxt.ExchangeError) as exc:
    ...
    for exchange_id in ("binance", "binanceus"):
        ...
        try:
            ...
            if exchange_id == "binance":
                logger.info("Using primary exchange binance.")
            else:
                logger.warning("Falling back to binanceus because Binance global is unavailable.")
            return exchange
            ...
            last_error = exc
    raise last_error or RuntimeError("Unable to load exchange markets.")
...
def analyze_symbol(exchange: ccxt.Exchange, symbol: str) -> dict[str, Any] | None:
    ...
    if not (daily_ok and macd_ok and volume_ok and rsi_ok and vol_up_ok):
        return None
...
if __name__ == "__main__":
    try:
        ...
        raise
from __future__ import annotationsimport loggingimport timefrom typing import Anyimport ccxtimport configfrom indicators import (    add_ema_columns,    calculate_ema10_slope_pct,    calculate_growth_score,    calculate_macd_values,    calculate_rsi_pair,    calculate_volume_ratio,    is_daily_bullish,    prepare_ohlcv_df,)from state_manager import (    get_alert_state,    should_send_only_new,    update_alert_state,)from telegram_sender import send_telegram_messagelogging.basicConfig(    level=logging.INFO,    format="%(asctime)s | %(levelname)s | %(name)s | %(message)s",)logger = logging.getLogger("binance_usdc_scanner")def with_retries(func, *args, **kwargs):    delay = config.INITIAL_RETRY_DELAY
